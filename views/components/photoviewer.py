...
class PhotoViewer(QtWidgets.QGraphicsView):
    ...
    def setPhoto(self, pixmap: Optional[QtGui.QPixmap] = None):
        ...
        if not (self.zoomPinned() and self.hasPhoto()):
            self._zoom = 0
        self.resetView(int(SCALE_FACTOR ** self._zoom))
    ...
    def updateCoordinates(self, pos: QtCore.QPointF | None = None):
        if self._photo.isUnderMouse():
            if pos is None:
                pos = QtCore.QPointF(self.mapFromGlobal(QtCore.QPointF(QtGui.QCursor.pos())))
            point = self.mapToScene(pos.toPoint()).toPoint()
        else:
            point = QtCore.QPoint()
        self.coordinatesChanged.emit(point)

    def mouseMoveEvent(self, event):
        # event.position() คืนค่า QPointF ซึ่งตรงกับ Type Hint ใหม่
        self.updateCoordinates(event.position())
